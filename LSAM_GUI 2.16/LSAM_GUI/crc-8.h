/*
 * crc.h
 *
 *  Created on: 2021. 7. 21.
 *      Author: tychoi
 */
#ifdef __cplusplus
extern "C" {
#endif

#include <stdio.h>
#include <math.h>
#ifndef _crc8_
#define _crc8_

static unsigned char crc_table[256] = {
    
}; // crc8LUT


#endif

/********************************************************************************
                                CONSTANTS
********************************************************************************/
#define	CRC8_POLY		0x18
#define	CRC8_INIT		0x00
#define	FAST_CRC8_INIT(crcReg)	(crcReg = CRC8_INIT)
#define	FAST_CRC8(crcData, crcReg)(crcReg = crc8LUT[crcData ^ crcReg])

unsigned char crc8_cal(unsigned char init_crc, const void* data, int length);

unsigned char crc8_cal(unsigned char init_crc, const void* data, int length) //init_crc : 0x00, *data : data address, length : data length
{
    unsigned char crc;
    const unsigned char* buf = (const unsigned char*)data;

    crc = init_crc;
    while (length--) {
        crc = crc_table[crc ^ *buf++];
    }
    return crc;
}

#ifdef __cplusplus
}
#endif